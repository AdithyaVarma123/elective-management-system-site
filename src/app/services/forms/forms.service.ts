...
export class FormsService {
    ...
    public getActiveForms(): Promise<IFormModel[]> {
        return new Promise<IFormModel[]>((resolve, reject) => {
            this.http.get(this.forms + 'active-forms').subscribe((res: IFormModel[]) => {
    ...
    public getAllForms(pageNumber: number): Promise<PaginationModel<IFormModel>> {
        return new Promise<PaginationModel<IFormModel>>((resolve, reject) => {
            this.http.get(this.forms + '?' + qs.stringify({ pageNumber, limit: 25 })).subscribe((res: PaginationModel<IFormModel>) => {
                const now = new Date().getTime();
    ...
    public updateForm(id: string, start: string, end: string, shouldSelect: number, selectAllAtForm: boolean, electives: string[]): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            this.http.post(this.forms, {
                id,
                start,
                end,
                shouldSelect,
    ...
    public respondToForm(id: string, electives: string[]): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            this.http.put(this.formsResponse, { id, electives }).subscribe(res => resolve(true), err => reject(err));
        });
    }

    public deleteForm(id: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            this.http.delete(this.forms + '?' + qs.stringify({ id })).subscribe(res => resolve(true), err => reject(err));
        });
    }
    ...
    public getResponses(id: string, pageNumber: number): Promise<PaginationModel<IResponseModel>> {
        return new Promise<PaginationModel<IResponseModel>>((resolve, reject) => {
            this.http.get(this.formsResponse + '?' + qs.stringify({ id, pageNumber, limit: 25 })).subscribe((res: PaginationModel<IResponseModel>) => {
                // @ts-ignore
