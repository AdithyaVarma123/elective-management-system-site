...
export class ClassesService {
    ...
    public getActiveClasses(): Promise<IClassModel[]> {
        return new Promise<IClassModel[]>((resolve, reject) => {
            this.http.get(this.classes + 'active').subscribe((res: IClassModel[]) => resolve(res), err => reject(err));
        });
    }
    ...
    public getClasses(page: number): Promise<PaginationModel<IClassModel>> {
        return new Promise<PaginationModel<IClassModel>>((resolve, reject) => {
            this.http.get(this.classes + '?' + qs.stringify({
                page,
                dir: 'asc',
                sortBy: 'batch'
            })).subscribe((res: PaginationModel<IClassModel>) => resolve(res), err => reject(err));
        });
    }

    public getStudents(id: string): Promise<IUserModel[]> {
        return new Promise<IUserModel[]>((resolve, reject) => {
            this.http.get(this.classes + 'students?' + qs.stringify({ id })).subscribe((res: IUserModel[]) => resolve(res), err => reject(err));
        });
    }
}
