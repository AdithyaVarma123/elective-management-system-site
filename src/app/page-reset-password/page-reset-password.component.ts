...
export class PageResetPasswordComponent implements OnInit {
  ...
  constructor(private user: UserService,private toast: ToastService,private router: Router,private _activatedRoute: ActivatedRoute) { }
  ...
  reset():void {
    if(this.newpass===this.confirmpass) {
      this._activatedRoute.queryParamMap.subscribe(parameter => {
        ...
        this.user.resetpass(this.newpass,this.code).then((result)=>{
          if(result)
            this.toast.green("email sent for reset");
        }).catch((err)=>{
